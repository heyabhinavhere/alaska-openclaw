...
### V4 → Cutover Era (June 4–12, 2026)

The month's big arc: V4 went from "activated" to "the graph is the source of truth," wrapped around an OpenClaw platform upgrade and the OM-4 command layer.

- **Cron thinning (Jun 4, #85–#91).** Crons were rewritten to "run the SKILL verbatim" so logic lives in git and auto-deploys (thin crons), instead of fat inline payloads that drift. Thinker, Daily Pulse, Pre-Call Brief thinned; Pre-Call Brief flipped to the SQLite-aware brief. The open-loop lesson: cron payloads live on the dashboard, not git — a SKILL edit doesn't reach a *fat* cron until it's manually thinned.
- **Standup-Reply Parser wired (Jun 4, #102/#104).** The pre-call-brief reply parser (`source=standup_reply`) got its own cron (it was built but dormant — no trigger). Added free-form reply handling + a numbered-reference resolver (anti-wrong-close).
- **OpenClaw upgrade 3.13 → 5.28 (Jun 5, #122–#130).** 3.13→5.26 crash-looped (the `@openclaw/slack` plugin needs ≥5.28); reverted, re-pinned to 5.28. Post-upgrade fallout: 5.28's model fallback went to openai (no key) → 14/19 crons + DMs broke until the default model was set to `anthropic/claude-sonnet-4-6` (#129); `plugins.entries.slack` had to be durably enabled (#130, a `doctor --fix` regression). **Lesson: 5.28 strips well-known credential env names (`GITHUB_TOKEN`/`GH_TOKEN`) from session env by design** — see the token rename below.
- **OM-4 `!`-command layer (Jun 5, #137–#144).** Native `/alaska` was built (#121–#135) then ABANDONED/reverted (#137) in favor of the `!`-command operating model: a STEP-0 router in SOUL recognizing `!case !audit !pmf !help !ping` + unambiguous bare verbs, with legacy `/` aliases. `command_audit` (migration 0007) logs routing for a measured promotion gate. Native Slack slash is deferred (postmortem 2026-06-05).
- **BON_GITHUB_TOKEN rename (Jun 12, #149).** Because 5.28 strips `GITHUB_TOKEN` by name, GitHub grounding was silently dead Jun 5–12. Renamed the env var to `BON_GITHUB_TOKEN` across code + skills + fat cron payloads. **Do NOT rename back.** (Only the var *name* changed — the token is still over-scoped read+write; the read-only scope swap is a separate, owner-held task.)
- **Bootstrap injection caps — the real number is 12k, not 20k (Jun 12, #151/#152).** The 5.28 source revealed the cap is **12,000 chars/FILE + 60,000 total** (over-budget files keep head 75% + tail 25% and SILENTLY DROP THE MIDDLE) — the old "~20k" lesson was 3.13-era and wrong. SOUL (23.3k) + MEMORY (17.5k) were being middle-truncated live. Fixed two ways: a 20k/80k config override (#151, safety net) AND restructuring SOUL→11.7k / MEMORY→11.1k under the 12k cap (#152), with `tests/test_workspace_budgets.py` as the forcing function.
- **THE CUTOVER (Jun 12, #156/#157).** The SQLite task graph became the source of truth. `DAILY_STATE.md` is now a hybrid view: MI writes the narrative; `lib/generate_daily_state.py` renders `## Per Person` / `## Active Blockers` from the graph after every MI run + Standup-Reply-Parser pass. #156 added `person_status` (migration 0008) + graph hygiene to reach parity first; #157 was the flip (parity verified twice — the first run found 4 P0s, fixed systemically; the re-run came back clean). MI no longer hand-writes the generated sections; on disagreement the graph wins.
- **Agent-memory self-task loop (Jun 12, #158; recall + scope #161/#162).** Closed the gap where `self_tasks` had no reader: a "Morning Self-Task Review" cron runs the agent-memory `review` op. Later phases added recall-quality + a team|builder two-notebook scope partition. (Forensics: the feature's recall wiring was never live Jun 3–12 because SOUL triggers were middle-truncated out of injection until #151/#152 fixed the caps.)
