...
- **Availability heard on the call → `person_status`.** "I'm traveling next week", "off Thursday–Friday", "back Monday" is a person-level fact, not a task: upsert it (`INSERT OR REPLACE INTO person_status (slack_id, status_text, until_date, set_by, updated_at) VALUES ('<their slack_id>', '<status>', <'ISO end' quoted — or SQL NULL UNQUOTED when no end date; never the string 'NULL'>, '<the reporting speaker's slack_id>', CURRENT_TIMESTAMP)` on `/data/queue/alaska.db` — `set_by` is ALWAYS a Slack ID per the schema contract, never a run id) so the generated Per-Person sections show a STATUS line instead of the person silently "looking available" (the Abhinav-traveling parity P0, 2026-06-12).
...
1. **Split the two questions — attribution vs attendance.** For **attribution of spoken items**, trust `speakers[].name` + the per-sentence `speaker_name` — never the `participants` metadata. For **attendance**, `participants` is the primary signal (silent attendees appear there but never in `speakers` — Daily Scrum rule, Step 3); but participants can ALSO undercount (shared-device joins; the May replay saw it missing most attendees), so absence-from-`participants` is WEAK evidence — it changes phrasing ("no spoken items on the call"), never a definitive "wasn't on the call". Dedupe duplicate speaker entries by normalized name (e.g. "Pankaj" and "Pankaj Pal" are one person).
