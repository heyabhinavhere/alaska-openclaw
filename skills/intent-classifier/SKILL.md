...
1. **Pre-filter (fast bypass, no LLM call):**
   ...
   - **Explicit `/alaska <subcommand>` command (route, don't classify):** if `message_text` (after any leading @-mention) starts with `/alaska`, it is a command-gateway command (e.g. `/alaska user 2762`, `/alaska help`, `/alaska ping`) — skip the 10-intent classification and route to the `command-gateway` skill with the rest of the message as the subcommand + args. It runs the deterministic `/alaska` executor and relays the reply; `/alaska user <id>` posts a 360° user case-file DOCX to the channel the command was run in (see `skills/command-gateway/SKILL.md`). Distinct from `/audit` — different command, different output.
