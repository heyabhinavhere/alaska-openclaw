...
1. **Pre-filter (fast bypass, no LLM call):**
   ...
   - **Commands are handled upstream — you never see them.** `!`-commands (`!case` / `!audit` / `!pmf` / `!help` / `!ping`) and their legacy `/pmf`, `/audit`, `/alaska` aliases are intercepted by `SOUL.md` → "STEP 0 — Command Router" *before* any classification. So every message that reaches you is NOT a command. A **bare** `audit <id>` / `pmf <q>` / `case <id>` (no `!`) is a normal message — classify it on its merits (usually `STATUS_QUERY` / `AMBIGUOUS`); do NOT treat it as a command or route it to the audit/pmf/case skill.
