...
- If a message is TRUNCATED, GARBLED, or column-misaligned (metadata bleeding into a field, cut off mid-word), do NOT write an analysis of the corruption — classify it AMBIGUOUS at confidence ≤ 0.3 with a ≤ 8-word reasoning and move on.
- KEEP OUTPUT TERSE: `reasoning` is ONE short sentence and `would_have_done` ONE short clause for EVERY row. Never exceed ~250 output tokens per row — verbose hedging on bad data is what blows the time budget (a corrupted-row run generated ~1,700 tokens/row vs ~200 for clean rows, and that output time alone hit the wall).
...
**This step ENDS IN AN ACTION, not a description.** For every row that passes the gate below you MUST actually invoke task-handler (and, for `TASK_ASSIGN`, also DM the assignee) before you finish this run. Writing `would_have_done` is the audit label, NOT the action — a gated row is not handled until task-handler has created/updated its task. **Forcing self-check before you finish the run:** count the rows that passed the gate as a task intent (`TASK_CREATE`/`TASK_UPDATE`/`TASK_BLOCKER`/`TASK_ASSIGN`); the number of task-handler invocations you made MUST equal that count. If any gated task row has no invocation, you skipped it — go back and act on it now. (`TASK_ASSIGN` is the one most easily skipped — it is NOT optional and NOT a "maybe later" path.)

For each row just classified, take a task action when ALL of these hold:
1. `intent ∈ {TASK_CREATE, TASK_UPDATE, TASK_BLOCKER, TASK_ASSIGN}` — **all four act, with equal weight** (`TASK_CREATE`/`TASK_UPDATE`/`TASK_BLOCKER` use the generic invocation just below; `TASK_ASSIGN` uses the assigner/assignee variant in the labelled arm below — same gated step, same run, equally mandatory), AND
...
For a `TASK_CREATE` / `TASK_UPDATE` / `TASK_BLOCKER` row that passes the gate, invoke the **task-handler** skill with (a `TASK_ASSIGN` row that passes the gate goes to the **TASK_ASSIGN arm** below instead — equally required, do not skip it):
...
2. **Pre-filter** the rows with the fast no-LLM bypasses above (bot self-messages, trivially short, emoji/punct-only). Mark each `processed=1` with its trivial intent immediately — they never reach the LLM. (No `classifier_audit` row is written for these deterministic bypasses; auditing them would only add noise to the Phase-A eval set, so only the `intent_inbox` UPDATE is applied.)
